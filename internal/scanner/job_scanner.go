...
func (js *JobScanner) considerCreatingRunners(queuedJobs []QueuedJobInfo) {
	...
	for repoKey, jobs := range jobsByRepo {
		...
		// Get current total runner count 
		repoFullName := fmt.Sprintf("%s/%s", repoOwner, repoName)
		status := js.runnerManager.GetStatus()
		runnersInfo, ok := status["runners"].(map[string]interface{})
		if !ok {
			log.Printf("Failed to get runners info for %s, skipping", repoFullName)
			continue
		}
		activeRunners, ok := runnersInfo["current"].(int)
		if !ok {
			log.Printf("Failed to get current runner count for %s, skipping", repoFullName)
			continue
		}
		
		// Get owner configuration to determine max runners
		ownerConfig, err := js.config.GetOwnerForRepo(repoFullName)
		if err != nil {
			log.Printf("No owner configuration found for %s, skipping runner creation", repoFullName)
			continue
		}
		
		maxRunners := ownerConfig.GetEffectiveMaxRunners(js.config.MaxRunners)
		
		// Calculate how many additional runners we can create
		availableSlots := maxRunners - activeRunners
		if availableSlots <= 0 {
			log.Printf("Repository %s already at max runners (%d/%d), not creating additional runners", 
				repoFullName, activeRunners, maxRunners)
			continue
		}

		// Limit runner creation to available slots and queued job count
		runnersToCreate := len(jobs)
		if runnersToCreate > availableSlots {
			runnersToCreate = availableSlots
		}

		log.Printf("Repository %s has %d queued compatible jobs, %d active runners (%d max), creating %d new runners", 
			repoFullName, len(jobs), activeRunners, maxRunners, runnersToCreate)

		// Create runners (up to the limit)
