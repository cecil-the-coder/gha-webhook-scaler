...
func (th *TestHandler) HandleWebhook(c *gin.Context) {
	...
	// Simple verification for test
	if th.config.WebhookSecret != "" {
		signature := c.GetHeader("X-Hub-Signature-256")
		if !th.verifySignature(body, signature, th.config.WebhookSecret) {
...
func (th *TestHandler) GetStatus(c *gin.Context) {
	...
	if th.config.AllRepos {
		if distribution, err := th.runnerManager.GetRunnerDistribution(); err == nil {
			status["runner_distribution"] = distribution
		}
		if repos, err := th.runnerManager.GetRepositories(); err == nil {
