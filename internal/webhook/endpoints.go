...
func NewEndpoints(config *config.Config, githubClients map[string]*github.Client) *Endpoints {
	...
	if defaultClient == nil && config.GitHubToken != "" {
		defaultClient = github.NewClient(config.GitHubToken)
	}
...
func (we *Endpoints) PreviewWebhookSetupHandler(c *gin.Context) {
	...
	} else if we.config.RepoOwner == owner {
		allRepos = we.config.AllRepos
	} else {
		c.JSON(http.StatusBadRequest, gin.H{
...
func (we *Endpoints) getGitHubClientForOwner(owner string) *github.Client {
	...
	// Fall back to legacy configuration if it matches
	if we.config.RepoOwner == owner && we.config.GitHubToken != "" {
		return github.NewClient(we.config.GitHubToken)
	}
