...
func (wh *Handler) HandleWebhook(c *gin.Context) {
	...
	if err != nil {
		// Check global webhook secret as fallback
		if wh.config.WebhookSecret != "" {
			signature := c.GetHeader("X-Hub-Signature-256")
			if !wh.verifySignature(body, signature, wh.config.WebhookSecret) {
				log.Printf("Invalid webhook signature for %s", prelimPayload.Repository.FullName)
...
func (wh *Handler) handleWorkflowJobEvent(body []byte) {
	...
	case "completed":
		...
		if err := wh.runnerManager.HandleWorkflowCompleted(payload.Repository.FullName, repoName); err != nil {
			log.Printf("Failed to handle job completed: %v", err)
		}
...
func (wh *Handler) handleWorkflowRunDirect(body []byte) {
	...
	case "queued":
		...
		log.Printf("Workflow queued, scaling up")
	...
	case "requested":
		...
		if payload.WorkflowRun.Status == "queued" {
			...
			log.Printf("Workflow requested and queued, scaling up")
		} else {
...
func (wh *Handler) GetStatus(c *gin.Context) {
	...
	// Add runner distribution information if in all-repos mode
	if wh.config.AllRepos {
