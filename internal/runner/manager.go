...
func (rm *Manager) ScaleUpWithLabels(reason string, repoFullName string, repoName string, customLabels []string) error {
	...
	if currentCount >= maxRunners {
		log.Printf("Cannot scale up for owner %s: already at max runners (%d)", ownerConfig.Owner, maxRunners)
		return nil
	}
	...
	log.Printf("Scaling UP: Starting new runner %s for repo %s with labels %v (reason: %s)", runnerName, repoFullName, labels, reason)

	// Rate limit runner creation - wait for permission to create a runner
	log.Printf("Rate limiting: waiting for permission to create runner %s", runnerName)
...
func (rm *Manager) GetStatus() map[string]interface{} {
	...
	if rm.config.MultiOwnerMode {
		...
		for owner, ownerConfig := range rm.config.Owners {
			...
			break // Take the first (and only) owner
		}
	}
	...
	return map[string]interface{}{
		...
		"config": map[string]interface{}{
			...
			"multi_owner_mode":   rm.config.MultiOwnerMode,
