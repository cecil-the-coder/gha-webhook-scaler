...
func (rm *Manager) ScaleUpWithLabels(reason string, repoFullName string, repoName string, customLabels []string) error {
	...
	// Calculate how many runners we can start
	availableSlots := maxRunners - currentCount
	if availableSlots <= 0 {
		...
		log.Printf("Cannot scale up for owner %s: already at max runners (%d). %d jobs waiting.", ownerConfig.Owner, maxRunners, jobCount)
	...
	for i := 0; i < availableSlots; i++ {
		...
		log.Printf("Scaling UP: Starting new runner %s for repo %s with labels %v (reason: %s, %d/%d)",
			runnerName, repoFullName, labels, reason, i+1, availableSlots)
	...
	// If we started fewer runners than available slots, there might be rate limiting or other issues
	if startedCount < availableSlots {
		remainingJobs := rm.getQueuedJobCount(githubClient, ownerConfig.Owner, repoName)
		if remainingJobs > 0 {
			rm.queueMemory.AddPendingRequest(repoFullName, repoName, reason, labels, remainingJobs)
			log.Printf("SCALE UP: Started %d/%d available runners. %d jobs still waiting for capacity.",
				startedCount, availableSlots, remainingJobs)
		}
	}
