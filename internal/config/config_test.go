...
func TestNewConfig(t *testing.T) {
	...
	envVars := []string{
		...
		"CACHE_VOLUMES", "MULTI_OWNER_MODE", "ALL_REPOS",
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if config.GitHubToken != tt.expected.GitHubToken {
				t.Errorf("GitHubToken = %v, want %v", config.GitHubToken, tt.expected.GitHubToken)
			}
			if config.RepoOwner != tt.expected.RepoOwner {
				t.Errorf("RepoOwner = %v, want %v", config.RepoOwner, tt.expected.RepoOwner)
			}
			if config.RepoName != tt.expected.RepoName {
				t.Errorf("RepoName = %v, want %v", config.RepoName, tt.expected.RepoName)
			}
...
func TestConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid single owner config",
			config: &Config{
				GitHubToken:    "token",
				RepoOwner:      "owner",
				RepoName:       "repo",
		...
		{
			name: "empty owners in single owner mode",
			config: &Config{
				GitHubToken:    "token",
				RepoOwner:      "owner",
				RepoName:       "repo",
...
func TestConfig_loadMultiOwnerConfig(t *testing.T) {
	// Save original environment
	originalEnv := make(map[string]string)
	envVars := []string{
	...
	tests := []struct {
		...
		},
		{
			...
			verify: func(t *testing.T, c *Config) {
				if len(c.Owners) != 0 {
					t.Errorf("Expected 0 owners, got %d", len(c.Owners))
				}
			},
		},
		{
...
func TestMultiOwnerMode_Integration(t *testing.T) {
	// Save original environment
	originalEnv := make(map[string]string)
	envVars := []string{
		"MULTI_OWNER_MODE", "OWNERS", "OWNER_TESTORG_GITHUB_TOKEN", 
	...
	// Set up multi-owner environment
	os.Setenv("MULTI_OWNER_MODE", "true")
	...
	// Test that multi-owner mode is enabled
	if !config.MultiOwnerMode {
		t.Error("Expected MultiOwnerMode to be true")
	}
