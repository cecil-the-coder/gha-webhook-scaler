...
func (k *KubernetesRuntime) StartRunnerWithLabelsForOwner(config *config.Config, ownerConfig *config.OwnerConfig, runnerName, repoFullName string, labels []string) (*RunnerContainer, error) {
	...
	pod := &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			...
			HostUsers:    &[]bool{false}[0],           // Enable user namespaces for rootless Docker
...
func (k *KubernetesRuntime) CleanupCompletedRunners(architecture string) error {
	...
	cleanedCount := 0
	for _, pod := range pods.Items {
		// Only clean up completed and succeeded pods
		if pod.Status.Phase == corev1.PodSucceeded || pod.Status.Phase == corev1.PodFailed {
			log.Printf("Cleaning up completed runner pod: %s (status: %s)", pod.Name, pod.Status.Phase)
...
func (k *KubernetesRuntime) buildContainers(requiresDocker bool, runnerName, repoURL, repoFullName, labelsString string, ownerConfig *config.OwnerConfig, config *config.Config) []corev1.Container {
	...
	if requiresDocker {
		log.Printf("Including Docker-in-Docker sidecar for runner %s (docker label detected)", runnerName)
		dindContainer := corev1.Container{
			...
			Command: []string{
				...
				"chmod 755 /tmp/podman && " +
				"echo 'Configuring Podman storage driver...' && " +
				"echo '[storage]' > /tmp/podman/storage.conf && " +
				"echo 'driver = \"vfs\"' >> /tmp/podman/storage.conf && " +
				"echo 'Starting Podman system service with debug output...' && " +
				"podman --log-level=debug system service --time=0 unix:///tmp/podman/podman.sock 2>&1 & " +
				"PODMAN_PID=$! && " +
				"echo 'Podman PID: $PODMAN_PID' && " +
				"echo 'Waiting for socket to be created...' && " +
				"for i in $(seq 1 60); do " +
				"  if [ -S /tmp/podman/podman.sock ]; then " +
				"    echo 'Socket created after ${i} seconds!' && " +
				"    chmod 666 /tmp/podman/podman.sock && " +
				"    echo 'Podman service ready!' && " +
				"    break; " +
				"  fi; " +
				"  if ! kill -0 $PODMAN_PID 2>/dev/null; then " +
				"    echo 'ERROR: Podman process died!' && " +
				"    exit 1; " +
				"  fi; " +
				"  echo 'Waiting... (${i}/60)' && " +
				"  sleep 1; " +
				"done && " +
				"if [ ! -S /tmp/podman/podman.sock ]; then " +
				"  echo 'ERROR: Socket not created after 60 seconds!' && " +
			...
			Env: []corev1.EnvVar{
				...
				{Name: "CONTAINERS_STORAGE_CONF", Value: "/tmp/podman/storage.conf"},
			},
		...
		log.Printf("Skipping Docker-in-Docker sidecar for runner %s (no docker label detected)", runnerName)
	}
	...
	// Add DOCKER_HOST only if Docker is required (Podman compatible)
	if requiresDocker {
	...
	runnerContainer := corev1.Container{
		...
		SecurityContext: &corev1.SecurityContext{
			...
			AllowPrivilegeEscalation: &[]bool{false}[0], // Required for Pod Security restricted policy
			Capabilities: &corev1.Capabilities{
				Drop: []corev1.Capability{"ALL"}, // Drop all capabilities for security
			},
			SeccompProfile: &corev1.SeccompProfile{
				Type: corev1.SeccompProfileTypeRuntimeDefault, // Use RuntimeDefault for security compliance
			},
		},
		Resources: corev1.ResourceRequirements{
			Requests: corev1.ResourceList{
	...
	// Add readiness probe only if Docker is required (to wait for Podman API)
	if requiresDocker {
		runnerContainer.ReadinessProbe = &corev1.Probe{
			ProbeHandler: corev1.ProbeHandler{
				Exec: &corev1.ExecAction{
					Command: []string{"sh", "-c", "test -S /tmp/podman/podman.sock && DOCKER_HOST=unix:///tmp/podman/podman.sock timeout 5 docker version --format '{{.Server.APIVersion}}' > /dev/null 2>&1"},
				},
			},
...
func (k *KubernetesRuntime) buildVolumes(requiresDocker bool) []corev1.Volume {
	...
	// Add Docker-related volumes only if Docker is required (now using Podman)
	if requiresDocker {
		volumes = append(volumes,
			corev1.Volume{
				Name: "docker-sock",
				VolumeSource: corev1.VolumeSource{
					EmptyDir: &corev1.EmptyDirVolumeSource{}, // Shared volume for Podman socket
				},
			},
			corev1.Volume{
				Name: "docker-lib",
				VolumeSource: corev1.VolumeSource{
					EmptyDir: &corev1.EmptyDirVolumeSource{}, // Podman container storage
				},
			},
		)
	}
	...
	return []corev1.Container{
		{
			...
			Command: []string{
				"sh", "-c",
				"mkdir -p /tmp/podman && chmod 755 /tmp/podman && chown 1000:1000 /tmp/podman",
			},
			SecurityContext: &corev1.SecurityContext{
...
// requiresDocker checks if "docker" is present in the workflow job labels
