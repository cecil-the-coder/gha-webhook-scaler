...
func (app *Application) logStartupInfo() {
	...
	if app.Config.AllRepos {
		log.Printf("Repository scope: All repositories under %s", app.Config.RepoOwner)
	} else {
		log.Printf("Repository: %s/%s", app.Config.RepoOwner, app.Config.RepoName)
	}
	...
	log.Printf("Webhook secret configured: %t", app.Config.WebhookSecret != "")
