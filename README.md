...
Enable support for multiple GitHub owners/organizations:

```bash
# Enable multi-owner mode
MULTI_OWNER_MODE=true
...
**Note:** Owner names with hyphens are converted to underscores in environment variables (e.g., `my-org` becomes `OWNER_MY_ORG_GITHUB_TOKEN`).
